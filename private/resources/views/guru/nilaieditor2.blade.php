            ...
            <div class="row-fluid">
                <div class="span12">
                    ...
                    <?php $bidangstudi=""; $bidangstudilength=0; $statusbidangstudi=0; $nilai=""; $nilailength=0; $statusnilai=0; $keterangan=""; $keteranganlength=0; $statusketerangan=0; ?>
                                @if(!$detailrapot -> first() -> bidang_studi_2 == '')
                                        <?php $statusbidangstudi = 1;
                                              $bidangstudi = explode("|", $detailrapot->first()->bidang_studi_2);
                                              $bidangstudilength = count($bidangstudi); ?>
                                @else
                                        <?php $statusbidangstudi = 0; ?>
                                @endif

                                @if(!$detailrapot -> first() -> semester_2 == '0')
                                        <?php $statusnilai = 1;
                                              $nilai = explode("|", $detailrapot->first()->semester_2);
                                              $nilailength = count($nilai); ?>
                                @else
                                        <?php $statusnilai = 0; ?>
                                @endif

                                @if(!$detailrapot -> first() -> keterangan_2 == '0')
                                        <?php $statusketerangan = 1;
                                              $keterangan = explode("|", $detailrapot->first()->keterangan_2);
                                              $keteranganlength = count($keterangan); ?>
                                @else
                                        <?php $statusketerangan = 0; ?>
                                @endif
                    <div class="paneleverything" style="margin-top: 10px;">
                        <form method="POST" enctype="multipart/form-data" action="../../../setnilai2">
                        ...
                        <table class="table table-bordered table-hover" width="100%">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{{ $bidangstudi[$i] }}</td>
                                    @if($statusnilai==1)
                                    <td style="text-align:center;"><input style="text-align:right" class="span8" type="text" name="skor2[]" value="{{ $nilai[$i] }}"  required></td>
                                    @else
                                    <td style="text-align:center;"><input style="text-align:right" class="span8" type="text" name="skor2[]" value='0'  required></td>
                                    @endif

                                    @if($statusketerangan==1)
                                    <td><textarea name="keterangan2[]" required>{{ $keterangan[$i] }}</textarea></td>
                                    @else
                                    <td><textarea name="keterangan2[]"  required></textarea></td>
                                    @endif
                                </tr>
                                @endfor
                                @else
